...
- secret 密码
- gcode 谷歌验证码
- code 手机/邮箱验证码,以注册方式为准，判断用户的 regtype,0 表示邮箱，1 表示手机。发送验证码请使用 /web/apisecurity/sendcode，会自动判断用户的注册方式发送相应的验证码。
...
- newpw 新登录密码
- renewpw 重复新登录密码
- code 手机验证码，根据用户信息的regtype判断，如果是0，使用邮箱验证码，如果是1，使用手机验证码。发送验证码请使用 /web/apisecurity/sendcode，会自动判断用户的注册方式发送相应的验证码。
- ga 谷歌验证码（如果开启了谷哥验证）
...
- email 是否设置了邮箱
- mobile 是否设置了手机
- ga 谷歌验证码设置，bind 表示是否设置了 ga，0 表示未设置，1 表示已设置；open 只有在 bind 为 1 的时候才有意义，表示是否开启验证
- password 是否设置了登录密码
- assettoken 是否设置了交易密码
- level 实名认证等级, 0: 未认证，1: 初级认证, 2: 高级认证, 3: 视频认证(尚无)
...
- idtype_desc 用户初级认证类型，前端可直接使用
- idnumber 打码的身份证号，未完成初级实名认证显示为空
- realname 打码的真实姓名，未完成初级实名认证显示为空
- nation 国家码
- nationname 初级实名认证时所选择的国家，未完成初级实名认证显示为空
